...
| travel group | 4,501 | **60.7%** | 51.5% | 9.2pp |
...
transported wholesale. The ordering is consistent: **exact cabin > travel group > surname**.
Note this does not cleanly separate location from relationship — cabin-mates are mostly a subset
of groupmates (finding 9) and are also the closest social tie — so read it as the pattern, not a
mechanism.
...
55.1% of all passengers travel solo. They were transported at **45.2%**; anyone in a group,
at **56.7%**. The rate climbs with group size to a peak at size 4 (64.1%) and then falls back
...
## 3. CryoSleep implies zero spend — with no exceptions in 12,970 rows

`figures/04_cryo_zero_spend.png`

Every one of the 4,581 frozen passengers spent exactly 0 on all five services. Zero violations.
This is a hard rule of the data generator, and it has two consequences:
...
Across 9,124 multi-member groups and 2,400 surnames: **zero violations.** Same for
**Side** — a travel group is always on one side of the ship (9,119 groups, zero violations).
Deck is *not* constant (673 violating groups), and neither is Destination (49.9%) or
CryoSleep (42.3%).
...
Of 1,157 passengers aged 0–12, **not one** has a nonzero charge on any service. At age 13
exactly, 53% of 219 passengers do. This is a discrete rule, not a smooth trend.

The age effect is not just a CryoSleep artefact. Among **awake** passengers only, under-13s were
transported at **68.2%** vs **30.5%** for awake adults — a 37.8pp gap, *larger* than the
unstratified one. Ages 18–65 are flat at 46–49%; the entire age signal lives under 18.
...
## 7. Starboard beats port on every single deck

`figures/03_side_by_deck.png`

Overall **S 55.5% vs P 45.1%** (+10.4pp), and the direction holds on all seven populated decks
without exception — A +11.1, B +11.0, C +18.3, D +6.2, E +2.9, F +5.9, G +13.6pp. A consistent
sign across seven independent strata is much stronger evidence than the single overall gap.
...
Cramér's V(deck, HomePlanet) = **0.75**. Decks A, B, C and T are 100% Europa; deck G is 100%
Earth; only D, E, F are mixed. So "deck matters" and "home planet matters" are **substantially
the same finding** and should not be counted twice.
...
Of 2,068 multi-member groups with a cabin, **1,397 (67.6%) occupy one identical cabin.** The
remaining 671 are not merely next door — **exactly 1** has a cabin-number span of ≤2, the median
split group spans 254 cabin numbers, and **all 671 span more than one deck.** There is no
"adjacent cabin" case at all: a group either shares a single cabin or is scattered across the
ship. (An earlier draft called these groups "adjacently booked" — that was wrong.)

The number is therefore a position, not an id, and it carries its own signal: across
cabin-number deciles *within* a deck, the rate moves 0.363 on deck G (37.1% → 73.4%) and 0.240
on deck C.
...
VIP is **2.1% of the ship** (273 of 12,970; 199 in train). VIPs were transported at 38.2% vs
...
Origin itself is stronger and clean: **Europa 65.9%, Mars 52.3%, Earth 42.4%.**
...
4. Add group-level aggregates (size, groupmate spend, groupmate cryo) — finding 1 says a
   companion's attributes predict yours.
5. Don't double-count deck and home planet as independent evidence.
